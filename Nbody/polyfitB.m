function [p,S,mu] = polyfitB(x,y,degree,b)
% POLYFITB Fit polynomial to data, forcing y-intercept to zero.
%   P = POLYFITB(X,Y,N,B) is similar POLYFIT(X,Y,N) except that the
%   y-intercept is forced to B, i.e. P(N) = B. In the same way as
...
%
%   [P,S,MU] = POLYFITB() Return structure, S, similar to POLYFIT for use
%   with POLYVAL to calculate error estimates of predictions with P.
%
%   [P,S,MU] = POLYFITB() Scale X by std(X), returns MU = [0,std(X)].
%
